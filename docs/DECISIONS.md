...
| 73 | **Handmatig opstellen en bewerken van documenten gebeurt in een Word-achtige editor (Tiptap) op een A4-pagina** met werkbalk (koppen 1-3, vet, cursief, opsommingen, genummerde lijsten, kaders, tabellen) in plaats van een markdown-tekstvak. De inhoud wordt bij opslaan vertaald naar het bestaande `StructuredDocument`-formaat (`editor-convert.ts`); vet en cursief worden als lichte inline-markering (`**`/`_`) opgeslagen en door de HTML-, DOCX- en PDF-renderers als runs weergegeven. Bewerken van een AI-concept maakt een nieuwe versie met `generatedBy: mens`; de bronnen van de vorige versie blijven behouden. | Eén opslagformaat voor AI- en mensdocumenten, met een vertrouwde bewerkervaring. |
